...
Berikut adalah hasil pencapaian penelitian tersebut untuk saat ini:

\begin{enumerate}

    \item Mengembangkan algoritma persepsi untuk mengenali objek dan
    ...
    \item Mengembangkan algoritma perencanaan jalur untuk \textit{decision
    ...
    \item Mengambil \textit{dataset} lingkungan trem berupa citra kamera biasa,
    citra LIDAR, dan data lainnya kemudian diolah dengan penambahan anotasi
    seperlunya.

    \item Mengembangkan \textit{Driving Assistance System} untuk trem berupa:
    ...
    \item Melakukan pengujian, analisis, dan desain ulang algoritma yang
    dikembangkan pada \textit{Software-in-the-Loop Simulation} (SILS) dan
    ...
    (EBS), dan \textit{Collision Avoidance} (\textit{decision making}).

    Dari pengujian dan analisis tersebut, dibutuhkan penyempurnaan algoritma
    persepsi berbasis sensor, perbaikan arsitektur (HILS), dan integrasi objek
    lokal pada skenario simulasi. Tugas Akhir ini mengimplementasikan
    mengintegrasikan objek-objek lokal dalam simulasi.

    \item Mengembangkan dan manufaktur \textit{platform} trem, sistem
    \textit{drive-by-wire} pada trem, dan integrasi sensor.
    \item Melakukan publikas ilmiah.
    \item Membuat draf kekayaan intelektual.
    \item Melakukan (\textit{self-assessment}) Tingkat Kesiapan Teknologi (TKT).
    \item Menyusun poster ilmiah populer atas pelaksanaan dan hasil riset.
