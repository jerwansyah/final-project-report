...
objek statis (bukan kendaraan) dan mengetes kendaraan pengganti trem sebagai
kendaraan utama yang dipasangkan sensor (\textit{ego vehicle}) dalam simulasi.
Aset model 3D sudah dibangun namun belum diimplementasikan ke dalam simulasi.
Aset model 3D tersebut adalah: trem, angkot, becak, sepeda onthel, sepeda motor,
beberapa rambu lokal, dan gerobak. Gambar \ref{fig:3d-model-assets} menunjukkan
aset model 3D yang telah dibuat.
...
% Aset seperti peta kota harus dibuat langsung menggunakan CARLAUE4 atau
% menggunakan RoadRunner kemudian diimpor ke dalam CARLAUE4 jika aset peta
% diperlukan.
...
modelling} seperti Blender untuk membuat aset.
...
% TODO: ? rencana import aset yang lain (yang khusus, yang detail di bawah aja)
% TODO: ? BP, factory, spline
% TODO: jelasin how to? but where

% (lihat Gambar \ref{fig:vehicle-skeleton}), aset dapat diperoleh dari
% \cite{blender-add-a-new-vehicle}
% \begin{figure}[ht]
%     \centering
%     \includegraphics[width=0.5\textwidth]{resources/chapter-3-vehicle-skeleton.png}
