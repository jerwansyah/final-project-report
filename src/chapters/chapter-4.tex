...
% TODO:
% same2 but different but still the same
% armature and bindings
...
% TODO: add preamble
% Hasil immplementasi selanjutnya divalidasi ketika simulasi dijalankan.

\subsection{Tujuan Validasi}

Tujuan validasi hasil implementasi adalah untuk memastikan bahwa objek-objek
non-statik atau objek yang dapat bergerak seperti kendaraan berperilaku normal
seperti kendaraan pada umumnya dan stabil saat simulasi berjalan. Berikut adalah
kriteria sukses validasi:

% TODO: lengkapin, statik ga divalidasi karna statik

\begin{enumerate}
    \item \dots
    % \item Kendaraan dapat dikemudikan (brake throttle steer) (program manual control.py --filter <nama_kendaraan>)
    % \item Kendaraan dapat berjalan ?? beroperasi ? dengan stabil? as expected? (pake program generate_traffic.py --filterv <nama_kendaraan>)
    % (essplain dijalanin perintah ^ pake -n 20 + -w 0 knp)
\end{enumerate}
