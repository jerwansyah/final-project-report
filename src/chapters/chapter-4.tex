...
Bagian ini membahas mengenai implementasi trem dan angkot sebagai kendaraan roda
4, sepeda onthel, sepeda motor, dan becak sebagai kendaraan roda 2, stasiun dan
rambu lalu lintas sebagai objek statis, dan rel sebagai objek statis dalam
bentuk \textit{spline}.
...
disebut sebagai \textit{folder} \verb|Blueprints|). Lima aset tersebut adalah aset 4
\textit{blueprint} roda dan 1 aset \textit{blueprint} kendaraan. Empat aset
\textit{blueprint} roda dibuat dengan klik kanan pada \textit{Content Browser}
dan memilih opsi \verb|Blueprint Class| kemudian memilih \verb|VehicleWheel|
untuk opsi \verb|Parent Class|. Masing-masing aset diberi nama
\verb|BP_<nama_kendaraan>_<posisi_roda>| dengan posisi roda: FLW (\textit{front
left wheel}), FRW (\textit{front right wheel}), RLW (\textit{rear left wheel}),
atau RRW (\textit{rear right wheel}). Aset-aset \textit{blueprint} roda dibuka
dan diatur nilai-nilainya pada panel \verb|Class Defaults| dengan konfigurasi
sebagai berikut:
...
\begin{figure}[!h]
    \centering
    \includegraphics[width=1\textwidth]{resources/chapter-4/tram-blueprint-wheels.png}
...
Selanjutnya, dilakukan \textit{spawning} kendaraan dengan menjalankan perintah
\verb|python3 manual_control.py --filter <nama_kendaraan>| pada terminal.
Program \verb|manual_control.py| terletak di dalam direktori
\verb|/PythonAPI/examples/|
